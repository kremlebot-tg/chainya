...
def test_public_typography_uses_distinct_roles_and_emphasizes_actions():
    assert "--w-body:    440;" in SOURCE
    assert "--w-heading: 520;" in SOURCE
    assert "--w-ui:      560;" in SOURCE
    assert "--w-strong:  650;" in SOURCE
    assert "--type-hero:    clamp(" in SOURCE
    assert "--type-section: clamp(" in SOURCE
    assert "font-family:var(--font-display); font-weight:var(--w-heading);" in SOURCE
    assert "font-family:var(--font-accent); font-size:.91em; font-weight:400;" in SOURCE
