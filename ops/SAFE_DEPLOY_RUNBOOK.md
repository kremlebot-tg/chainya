# Архивный runbook двухсерверного режима Chainya

> 01.09.2026 backend, Telegram-бот и единственная рабочая SQLite перенесены на
> Timeweb. Этот сценарий оставлен только как документация отката и не должен
> использоваться для обычного релиза. `deploy-shop.sh` намеренно заблокирован,
> чтобы случайно не запустить вторую рабочую базу на старом origin.
...
Если локальный SSH-псевдоним origin недоступен, цель можно передать только для
текущего запуска через `CHAINYA_ORIGIN_HOST`. Это не меняет DNS, SSH-конфиг или
production и не должно сохраняться в репозитории вместе с адресом сервера.
