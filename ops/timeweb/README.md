...
После переключения origin этот скрипт вызывает `deploy-edge.sh`: загружает
собранный `dist/` в новый каталог `/var/www/chainya-releases/`, проверяет его,
атомарно меняет ссылку `/var/www/chainya` и сверяет публичные страницы. При
ошибке во время переключения edge возвращается к предыдущему статическому
release. Отдельно запускать `deploy-edge.sh` нужно только для восстановления
рассинхронизированной статики; backend и интеграции он не меняет.
